 ...
 *
 * The transform belongs to the MOUNT. The rider is carried by it and has no
 ...
 */
export const MountAnimationState = {
  Idle: 'idle',
  Walk: 'walk',
  Gallop: 'gallop',
  JumpStart: 'jumpStart',
...
export type MountAnimationState =
  (typeof MountAnimationState)[keyof typeof MountAnimationState];

/**
 * The compact per-player signals a client needs to reconstruct another
 * player's animation locally.
 *
 * Bone transforms and mount part transforms are NEVER sent over the network -
...
export interface PlayerProgression {
  level: number;
  /** Completed rebirths. Drives the level cap and the Speed-gain multiplier. */
  rebirths: number;
  /** Stage wins collected. Awarded by the server only. */
  wins: number;
  /** Lifetime Speed farmed by riding. Awarded by the server only. */
  totalSpeed: number;
  /** Slot of the currently equipped mount - the highest one evolved into. */
  mountSlot: number;
  /** Bitmask of mounts unlocked, one bit per slot. */
  unlockedMounts: number;
  /** Equipped speed-upgrade pad, which sets the BASE Speed per step. */
  upgradeSlot: number;
   ...
   *
   * Resolved by the one shared gain formula and replicated, so the HUD's
   * "+N" popups show exactly what the server paid rather than a figure the
   * client worked out for itself.
   */
  speedPerStep: number;
  /** The product of every multiplier alone, for the HUD's "Total Multiplier". */
  ...
  trailSlot: number;
  /** Bitmask of auras bought. Written by the server only. */
  ownedAuras: number;
  /** Equipped aura slot, or 0 for none. */
  auraSlot: number;
}
