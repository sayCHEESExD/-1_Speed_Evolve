 ...
 *
 * What moves is the ANIMAL. The rider is carried: they have no velocity, no
 * collision and no state here, which is exactly why swapping the animal can
...
export interface PlayerMotion {
  ...
  jumpCount: number;
  /**
   * The treadmill that is RUNNING for this player, or 0.
   *
   * DERIVED from position and the player's own rebirth count every step by
   * both sides, never sent. Riding on starts it and riding off stops it, so
   * there is no message to forge and nothing to keep after stepping off. It
   * changes no physics at all - it only tells the Speed service to bill the
   * belt instead of the ground, and the animator to run on the spot.
   *
   * ZERO ON A LOCKED BELT, which is the gate. A player without the rebirths a
   * machine asks for stands on an inert floor: they are credited no belt
   * distance, they receive no tier multiplier, and their mount does not run on
   * the spot as though the machine were working.
   */
  treadmill: number;
  /**
   * The belt physically UNDERFOOT, locked or not, or 0.
   *
   * Separate from `treadmill` because the two answer different questions, and
   * the client needs both: this one is what puts "Requires 1 Rebirth" on
   * screen. Not replicated - it is pure geometry, so every side derives it
   * from a position it already has.
   */
  treadmillUnder: number;
  /**
...
export interface SimParams {
  ...
  jumpVelocity: number;
  /**
   * The player's authoritative rebirth count.
   *
   * Here rather than in `MovementInput` because it is SERVER-OWNED: a client
   * that could supply it could unlock the 3x belt by typing a number. The
   * client's copy comes from replicated state, so prediction reaches the same
   * answer the server does and stepping onto a locked belt mispredicts
   * nothing.
   */
  rebirths: number;
  /**
...
/**
 * Scratch for the platform carry.
 *
 * The one piece of state a moving platform needs from the simulation, and it
 * is reused rather than allocated because `integrate` runs dozens of times per
 * frame at late-game speeds.
 */
const CARRY: MotionPoint = { x: 0, y: 0, z: 0 };
...
export const stepPlayer = (
   ...
   *
   * TWO figures, because a locked belt is a real place the player can be
   * standing and a machine that is doing nothing for them. `treadmillUnder` is
   * where they are; `treadmill` is what is paying. Deriving only the first and
   * gating the payment downstream is what let a player with no rebirths farm
   * the 3x machine at the ordinary rate.
   */
  if (motion.grounded) {
    motion.treadmillUnder = treadmillAt(motion.x, motion.y, motion.z);
    motion.treadmill = activeTreadmillAt(motion.x, motion.y, motion.z, params.rebirths);
  } else {
    motion.treadmillUnder = 0;
    motion.treadmill = 0;
  }
...
const integrate = (motion: PlayerMotion, dt: number, collision: WorldCollision): void => {
  ...
  /*
   * Ride whatever is underfoot, BEFORE the player's own movement is applied.
   *
   * Without this a moving platform is a platform the player slides off: the
   * world moves and they do not. Applied as a displacement rather than as a
   * velocity, because a velocity would persist after stepping off - a player
   * who left a carousel would keep its tangential speed for ever, which is a
   * launcher rather than a platform.
   *
   * It runs inside the substep and therefore through the same collision
   * resolution the player's own movement does, so being carried into a wall
   * stops at the wall rather than pushing through it.
   */
  if (motion.grounded) {
    collision.carryAt(motion.x, motion.z, motion.y, dt, CARRY);
    motion.x += CARRY.x;
    motion.y += CARRY.y;
    motion.z += CARRY.z;
  }
