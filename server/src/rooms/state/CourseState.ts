...
import { GUARDIANS, LEADERBOARD_SIZE } from '@evolve/shared';
 ...
 * One guardian.
 *
 * The only things in this world replicated rather than derived, because they
 * CHASE - their positions depend on where the players are, which is state and
 * not a formula. The server owns every field here; the client draws them and
 * does nothing else with them.
 */
export class GuardianState extends Schema {
  @type('float32') x = 0;
  /** Its own arena's floor. The two guardians stand at different elevations. */
  @type('float32') y = 0;
  @type('float32') z = 0;
  @type('float32') rotationY = 0;
  /** True while it has noticed someone. Drives the run cycle and the roar. */
  @type('boolean') charging = false;
}

/**
 * One entry per guardian the WORLD declares, allocated once at construction.
 *
 * Sized from `GUARDIANS` rather than from a literal, so adding a third
 * guardian to a stage is an entry in the course and nothing here.
 */
const guardianRow = (): ArraySchema<GuardianState> => {
  const list = new ArraySchema<GuardianState>();
  for (let i = 0; i < GUARDIANS.length; i += 1) list.push(new GuardianState());
  return list;
};
...
export class CourseState extends Schema {
  ...
  @type([GuardianState]) guardians = guardianRow();
