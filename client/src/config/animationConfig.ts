...
export const GAIT = {
  ...
  /** Below this speed the mount is standing still. */
  idleSpeed: 0.6,
  ...
  /**
   * How far a gallop spreads its pairs out around the cycle.
   *
   * Scaled by the pair count in `gallopPhase` below, so a two-pair bound and
   * a four-pair one cover the same fraction of the cycle rather than the
   * spider's back legs ending up a whole revolution behind its front ones.
   */
  gallopSpread: 0.83,
  /** Offset between the two legs of one pair during a gallop. */
  gallopPairOffset: 0.4,
...
/**
 * Leg phase offsets, in radians, DERIVED from the leg count.
 *
 * A table would have to be written once per leg count the roster contains, and
 * the roster contains four of them - two legs, four, six and eight. Deriving
 * them instead is what lets a spider and a chick share the one animator.
 *
 * A walk is the ALTERNATING gait every legged animal actually uses: each pair
 * is out of phase with its neighbour, and each leg with its partner. At two
 * pairs that is the familiar diagonal trot; at three it falls out as the
 * alternating tripod a real cockroach walks with, which is a pleasant
 * accident of the same rule rather than a special case.
 *
 * A gallop bunches each pair together and spreads the pairs around the cycle,
 * so the creature bounds rather than trots faster. Blending BETWEEN the two is
 * what makes speeding up read as a change of gait.
 *
 * @param index 0-based leg, front pair first, right then left within a pair
 * @param count how many legs this species has
 */
export const trotPhase = (index: number, count: number): number => {
  const pair = Math.floor(index / 2);
  const side = index % 2;
  return (pair + side) * Math.PI;
};

export const gallopPhase = (index: number, count: number): number => {
  const pairs = Math.max(1, Math.round(count / 2));
  const pair = Math.floor(index / 2);
  const side = index % 2;
  const lag = ((Math.PI * 2) / pairs) * GAIT.gallopSpread;
  return pair * lag + side * GAIT.gallopPairOffset;
};

/** Crouch, launch, airborne and land. */
export const JUMP = {
  /** Seconds of crouch before the mount leaves the ground. */
  crouchDuration: 0.09,
   ...
   *
   * The ankle angles are what stop a jump reading as a model being carried
   * through the air. A tucked limb curls at every joint - a hoof that stays
   * flat under a folded knee is the detail that makes an airborne animal look
...
export const DEATH = {
  ...
  duration: 0.55,
  /** How far the mount keels over, in radians. */
  roll: deg(96),
...
export const RIDE = {
  ...
  /** Amplitude of the rider's own bounce against the mount's gait. */
  ...
  riseLean: deg(-11),
  fallLean: deg(9),
  /** Arms rise as the mount leaves the ground. */
  jumpArmLift: deg(-26),

  /** How far the rider slumps as the mount goes over. */
  deathSlump: deg(52),
} as const;
