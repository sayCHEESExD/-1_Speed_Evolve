 ...
 * shared so the animal responds instantly, keeps every input the server has
...
export class LocalPlayer {
  ...
  /** Steering this frame, for the mount's head turn. */
  private turnSignal = 0;

  private readonly animationInput: AnimationInput = createAnimationInput();

  constructor(collision: WorldCollision, mountSlot: number) {
    this.collision = collision;
    this.mount = new Mount(mountSlot);
  ...
  get animationState(): MountAnimationState {
   ...
   *
   * Resolved server-side from level, rebirth and the equipped animal;
  ...
  /**
   * Mirror the replicated rebirth count into the prediction.
   *
   * The simulation needs it to decide whether the belt underfoot is running.
   * It is REPLICATED rather than tracked locally for the usual reason: the
   * server owns it, and a prediction that used a client-side guess would let a
   * locked treadmill appear to work until the next correction.
   */
  setRebirths(rebirths: number): void {
    if (Number.isFinite(rebirths)) this.params.rebirths = Math.max(0, Math.floor(rebirths));
  }

  /** Show the mount the server says this player is riding. Cosmetic only. */
  setMountSlot(slot: number): void {
    this.mount.setMountSlot(slot);
  }
  ...
  setAuraSlot(slot: number): void {
    this.mount.setAuraSlot(slot);
  }
  ...
  /** True while the mount is on a treadmill that is actually RUNNING. */
  get onTreadmill(): boolean {
    return this.motion.treadmill > 0;
  }

  /**
   * The belt underfoot that this player has NOT unlocked, or 0.
   *
   * What the locked-machine prompt is driven from. It is the difference
   * between the two figures the simulation derives: standing on a belt that is
   * not running is exactly the state worth telling the player about.
   */
  get lockedTreadmill(): number {
    return this.motion.treadmill > 0 ? 0 : this.motion.treadmillUnder;
  }
