import { trailBySlot } from '@evolve/shared';
import {
  AdditiveBlending,
  BufferAttribute,
  BufferGeometry,
...
export class TrailEffect {
  ...
  /** The tier's SECOND colour, for the gradient and sparkle styles. */
  ...
  private rebuild(): void {
    ...
    const style = tier?.style ?? 'solid';
    // Resolved once per rebuild rather than per vertex: the second colour is a
    // property of the TIER, and looking it up inside the loop would be six
    // hundred map reads a frame for a number that cannot change within one.
    if (tier) this.tintB.setHex(tier.colorB);
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      // How a tier is COLOURED, and nothing else about it. Every style shares
      // the same ribbon, the same emission rule and the same taper - which is
      // what keeps twelve trails one effect with twelve looks rather than
      // twelve effects.
      if (style === 'rainbow') {
        this.tint.setHSL((this.time * 0.35 + t) % 1, 0.85, 0.6);
      } else if (style === 'gradient') {
        // Head colour at the mount, tail colour at the far end.
        this.tint.setHex(tier?.color ?? 0xffffff).lerp(this.tintB, 1 - t);
      } else if (style === 'sparkle') {
        // A travelling glint, so the ribbon twinkles without a particle system
        // and without a single extra vertex.
        const glint = Math.sin((t * 9 - this.time * 4) * Math.PI) > 0.72 ? 1 : 0;
        this.tint.setHex(tier?.color ?? 0xffffff).lerp(this.tintB, glint * 0.85);
      } else if (tier) {
