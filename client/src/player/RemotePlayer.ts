...
export class RemotePlayer {
  ...
  constructor(state: NetPlayerState) {
    this.mount = new Mount(state.mountSlot);
  ...
  apply(state: NetPlayerState): void {
    ...
    this.mount.setMountSlot(state.mountSlot);
    this.mount.setTrailSlot(state.trailSlot);
    // Remote auras are driven from the SAME replicated field the local one is,
    // so a player wearing a Rainbow aura looks identical on every screen in
    // the room including their own.
    this.mount.setAuraSlot(state.auraSlot);
