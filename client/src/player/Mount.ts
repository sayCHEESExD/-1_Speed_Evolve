import { mountForSlot, type MountAnimationState } from '@evolve/shared';
import { Group, Mesh, Object3D } from 'three';
import { MountModel } from '../mount/MountModel.js';
import { MountAnimator } from '../animation/MountAnimator.js';
...
export class Mount {
  ...
  /**
   * The glow the equipped aura surrounds the mount with.
   *
   * Parented to `root`, unlike the trail: an aura is where the player IS, so
   * it travels with them, where a trail is where they have BEEN and must stay
   * put in world space. That one difference is the whole reason there are two
   * effect roots on this class.
   */
  readonly aura = new AuraEffect();

  creature: MountModel;
  mountAnimator: MountAnimator;
  /**
  ...
  constructor(slot: number) {
    ...
    this.creature = new MountModel(mountForSlot(this.slot));
    this.root.add(this.creature.root);
    ...
    this.creature.riderAnchor.add(this.riderVisual);
    ...
    this.mountAnimator = new MountAnimator(this.creature);
   ...
   *
   * The mirror of `setMountSlot`, and for the same reason: the two halves are
  ...
  updateEffects(delta: number, x: number, y: number, z: number, speed: number): void {
    ...
    // The aura takes no position: it is parented to the mount's root, so it is
    // already wherever the mount is.
    this.aura.update(delta);
  }

  /** The mount the server says this player is riding. */
  get mountSlot(): number {
    return this.slot;
  }
  ...
  setMountSlot(slot: number): void {
    ...
    this.creature = new MountModel(mountForSlot(next));
    this.root.add(this.creature.root);
    this.creature.riderAnchor.add(this.riderVisual);
  ...
  setVisualScale(x: number, y: number, z: number): void {
    this.creature.root.scale.set(x, y, z);
  }
  ...
  update(delta: number, input: AnimationInput): void {
    this.mountAnimator.update(delta, input);
    this.riderAnimator.update(delta, input, this.mountAnimator.gaitPhase);
  }

  get animationState(): MountAnimationState {
    return this.mountAnimator.currentState;
  }
  ...
  resetAnimation(): void {
    this.mountAnimator.reset();
    this.riderAnimator.reset();
    this.creature.root.scale.set(1, 1, 1);
