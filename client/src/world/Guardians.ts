...
/**
 * Palette. Mossy grey-green, so it belongs to the overgrown grove it lives in
 * rather than looking like something dropped into it.
 */
const HIDE = 0x6f7a5e;
const HIDE_DARK = 0x56604a;
const TUSK = 0xf4efe2;
const PAD = 0x3f4736;
...
/**
 * ONE guardian: the thing that hunts the groves of stages 18 and 29.
 *
 * Built from the same `BoxSet` the rideable mounts use, so it is the same
...
class Guardian {
  ...
  constructor() {
    ...
    // head, the trunk and four legs is all the articulation a blocky animal
    // this size needs.
    ...
    // Ears: big flat plates, the fastest way to say "enormous animal" in boxes.
    head.addMirrored([1, 8, 7], [5, 0.5, -1], HIDE_DARK);
    // Eyes, in the same flat style as the rideable animals.
    head.addMirrored([1.1, 1.1, 0.4], [2.2, 1.8, 3.1], 0xffffff);
    head.addMirrored([0.5, 0.5, 0.35], [2.2, 1.8, 3.35], 0x1b1b22);
    // Tusks.
    ...
    // Placed by the first `apply`; until then it is hidden.
    this.root.position.set(0, 0, 0);
  }

  /** Copy the replicated transform in. Called on every patch. */
  apply(x: number, y: number, z: number, rotationY: number, charging: boolean): void {
    this.targetX = x;
    this.targetY = y;
    this.targetZ = z;
    this.targetYaw = rotationY;
    this.charging = charging;
    // Hidden until the server has actually told us where it is, so a guardian
    // never spends the first frame of a session standing at the world origin
  ...
  update(delta: number): void {
    ...
    for (let i = 0; i < this.legs.length; i += 1) {
      ...
      if (!hip) continue;
      // Diagonal pairs, as with the rideable animals.
...
/**
 * Every guardian the world declares, drawn.
 *
 * Sized from `GUARDIANS` rather than from a literal two, so a third guardian
 * placed in a stage appears here with no change at all. The room replicates
 * one state row per guardian, in the same order, and `apply` matches them up
 * by index - which is the whole reason both lists are derived from the course
 * data rather than authored twice.
 */
export class Guardians {
  readonly root = new Group();

  private readonly beasts: Guardian[] = [];

  constructor() {
    for (let i = 0; i < GUARDIANS.length; i += 1) {
      const beast = new Guardian();
      this.root.add(beast.root);
      this.beasts.push(beast);
    }
  }

  /** Place one guardian from its replicated row. */
  apply(
    index: number,
    x: number,
    y: number,
    ...
    this.beasts[index]?.apply(x, y, z, rotationY, charging);
  }

  update(delta: number): void {
    for (const beast of this.beasts) beast.update(delta);
  }

  dispose(): void {
    for (const beast of this.beasts) beast.dispose();
