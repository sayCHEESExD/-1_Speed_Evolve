...
/** Handed out before the first patch, so no caller allocates to find nothing. */
const EMPTY_GUARDIANS: readonly NetGuardianState[] = [];

/** Key under which this browser's stable player id is kept. */
const PLAYER_ID_KEY = 'speedevolve.playerId';
 ...
 * transport or the room name only touches this file and @evolve/shared.
 */
export class NetworkClient {
  ...
  /** Ask to equip the speed-upgrade pad the mount is standing on. */
  claimUpgrade(slot: number): void {
    const message: ClaimUpgradeMessage = { slot };
    this.room?.send(MessageType.ClaimUpgrade, message);
  }
  ...
  /** Ask to buy the aura in this slot. A request, never a claim. */
  buyAura(slot: number): void {
    this.room?.send(MessageType.BuyAura, { slot });
  }

  /** Ask to wear an owned aura, or 0 to take it off. */
  equipAura(slot: number): void {
    this.room?.send(MessageType.EquipAura, { slot });
  }

  /** Ask to buy the item in this slot. A request, never a claim. */
  buyItem(slot: number): void {
    this.room?.send(MessageType.BuyItem, { slot });
  }

  /** Ask to carry an owned item, or 0 to put it away. */
  equipItem(slot: number): void {
    this.room?.send(MessageType.EquipItem, { slot });
  }

  /**
   * The replicated guardians, in the order the world declares them.
   *
   * COPIED out of the schema rather than handed over live, for the same reason
   * the leaderboards are: a live reference reads as whatever it holds at the
   * moment it is looked at, and the renderer reads these once per frame while
   * patches arrive twenty times a second. Empty before the first patch.
   */
  get guardians(): readonly NetGuardianState[] {
    const rows = this.room?.state?.guardians;
    if (!rows) return EMPTY_GUARDIANS;
    const out: NetGuardianState[] = [];
