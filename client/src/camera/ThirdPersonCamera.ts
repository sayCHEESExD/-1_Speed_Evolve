import { CAMERA } from '@evolve/shared';
...
export class ThirdPersonCamera {
  ...
  /**
   * Extra distance and height the current mount's SIZE asks for.
   *
   * Eased rather than applied outright, because it changes at the moment of an
   * evolution - and a camera that jumped backwards on the same frame the mount
   * changed would make the reward read as a glitch.
   */
  private subjectPull = 0;
  private subjectPullTarget = 0;
  ...
  /**
   * Tell the camera how tall the mount is, feet to crown.
   *
   * The ONE input the framing takes from the roster, and it is a measurement
   * rather than a slot - so a new mount frames correctly without this file
   * learning it exists.
   */
  setSubjectHeight(height: number): void {
    if (!Number.isFinite(height) || height <= 0) return;
    this.subjectPullTarget = Math.max(0, height - CAMERA.subjectHeight) * CAMERA.subjectScale;
  }
  ...
  update(delta: number, speed: number): void {
    ...
    this.subjectPull +=
      (this.subjectPullTarget - this.subjectPull) * (1 - Math.exp(-4 * delta));
    ...
    const distance = Math.max(
      ...
      CAMERA.distance +
        ...
        this.zoomEased +
        this.zoomOffset +
        CAMERA.speedDistance * this.rush,
    );
    ...
    this.camera.position
      ...
      .add(OFFSET.set(0, CAMERA.height + this.subjectPull * 0.45 + sinPitch * distance, 0));

    // The look target rises with the mount too, or a dragon's rider would sit
    // at the top of the frame with the camera aimed at its knees.
    LOOK_TARGET.copy(this.followed).add(
      OFFSET.set(0, CAMERA.lookAtHeight + this.subjectPull * 0.3, 0),
    );
